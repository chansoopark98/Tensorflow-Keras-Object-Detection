...
# TODO(wider_face_dataset): Markdown description  that will appear on the catalog page.
_DESCRIPTION = """
...
# TODO(wider_face_dataset): BibTeX citation
_CITATION = """
"""


class WiderFaceDataset(tfds.core.GeneratorBasedBuilder):
  """DatasetBuilder for wider_face_dataset dataset."""
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    archive_path = '../data_generate/data/wider_face/wider_face_dataset.zip'
    extracted_path = dl_manager.extract(archive_path)

    # TODO(cornell_grasp): Returns the Dict[split names, Iterator[Key, Example]]
    return {
        'train': self._generate_examples(img_path=extracted_path/'image',
    ...
    for idx in range(len(img_files)):
      ...
      try:  
        for i in range(len(read_bbox_list)):
            ...
            read_label_list[i] = int(float(read_label_list[i]))
