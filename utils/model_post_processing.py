...
def merge_post_process(detections, target_transform, confidence_threshold=0.01, top_k=100, iou_threshold=0.5, classes=21):
    ...
    if not tf.is_tensor(batch_boxes):
        batch_boxes = tf.convert_to_tensor(batch_boxes)
    batch_scores = tf.nn.softmax(detections[:, :classes], axis=1)
    
    batch_boxes = convert_locations_to_boxes_frozen(batch_boxes, target_transform.center_form_priors,
                                             target_transform.center_variance, target_transform.size_variance)
    batch_boxes = center_form_to_corner_form(batch_boxes, axis=1)

    # batch_size = tf.shape(batch_scores)[0]
    results = []
    
    scores, boxes = batch_scores, batch_boxes  # (N, #CLS) (N, 4)
    ...
    results = tf.convert_to_tensor(output)
