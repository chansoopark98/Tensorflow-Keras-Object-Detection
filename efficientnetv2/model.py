...
class EffNetV2Model(tf.keras.Model):
  ...
  def summary(self, input_shape=(512, 512, 3), **kargs):
    x = tf.keras.Input(shape=input_shape)
    model = tf.keras.Model(inputs=[x], outputs=self.call(x, training=True))
    return model.summary()

  def call(self, inputs, training, features_only=True):
    """Implementation of call().
    Args:
    ...
    return self.endpoints.get('reduction_3'),self.endpoints.get('reduction_4'),self.endpoints.get('reduction_5')
...
def create_model(model_name, dataset_cfg, hparam_str=''):
    ...
    return EffNetV2Model(model_name, config.model)
...
def build_tf2_model():
    ...
    model = create_model(model_name='efficientnetv2-s',dataset_cfg='Imagenet')
                         #FLAGS.dataset_cfg, FLAGS.hparam_str)
    # Use call (not build) to match the namescope: tensorflow issues/29576


    ckpt = tf.train.latest_checkpoint('./datasets/efficientnetv2-s')
    model.load_weights(ckpt)
    output = model(tf.ones([1, 512, 512, 3]), True)
