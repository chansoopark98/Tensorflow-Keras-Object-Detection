...
def create_classifier(source_layers, num_priors, normalizations, num_classes=81, regularization=5e-4):
    mbox_conf = []
    mbox_loc = []
    for i, layer in enumerate(source_layers):
        ...
        x1 = Conv2D(num_priors[i] * num_classes, 3, padding='same', kernel_regularizer=l2(regularization),
                    name=name + '_mbox_conf')(x)
        x1 = Flatten(name=name + '_mbox_conf_flat')(x1)
        # x1 = activation_b5_mbox_conf_flat/Reshape:0 , shape=(Batch , 84)
        mbox_conf.append(x1)

        x2 = Conv2D(num_priors[i] * 4, 3, padding='same', kernel_regularizer=l2(regularization),
