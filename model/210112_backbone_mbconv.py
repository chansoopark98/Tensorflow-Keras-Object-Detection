...
def create_backbone(base_model_name, pretrained=True, IMAGE_SIZE=[300, 300], regularization=5e-4):
    ...
    conv10 = Conv2D(160, (1, 1), padding='same', name='conv10_channel_resizing', activation=tf.nn.relu6)(conv10)
    ...
    # fpn conv
    conv38 = cbam_block(conv38, 8)

    conv38_s = strideMBConv(conv38, 240, 40, 'conv38_stride_mbconv_1')
    conv19 = Concatenate()([conv38_s, conv19]) # 40+112
    conv19 = cbam_block(conv19, 8)
    conv19 = Conv2D(112, (1, 1), padding='same', name='conv19_channel_resizing', activation=tf.nn.relu6)(conv19)

    conv19_s = strideMBConv(conv19, 672, 112, 'conv19_stride_mbconv_1')
    conv10 = Concatenate()([conv19_s, conv10])
    conv10 = cbam_block(conv10, 8)
    conv10 = Conv2D(160, (1, 1), padding='same', name='conv10_channel_resizing_2', activation=tf.nn.relu6)(conv10)
