...
def concat_convolution(input_tensor, channel, size, stride, padding, name):
    ...
    emphasize = shared_dense(units=channel/8, activation='relu3', name=name+'_shared_dense_1')(emphasize)
...
def create_backbone(base_model_name, pretrained=True, IMAGE_SIZE=[300, 300], regularization=5e-4):
    ...
    conv19 = concat_convolution(conv19, 128, 3, 1, 'SAME', 'conv19_concat_conv')
    ...
    conv10 = concat_convolution(conv10, 256, 3, 1, 'SAME', 'conv10_concat_conv')
    ...
    # same = 20
    # valid = 21
    deconv_conv19 = deconvolution(conv10, 128, 19, 'deconv10_to_19')


    conv19 = Concatenate()([deconv_conv19, conv19])
    conv19 = concat_convolution(conv19, 256, 3, 1, 'SAME', 'conv19_concat_deconv')




    deconv_conv38 = deconvolution(conv19, 128, 38, 'deconv19_to_38')

    conv38 = Concatenate()([deconv_conv38, conv38])
    conv38 = concat_convolution(conv38, 160, 3, 1, 'SAME', 'conv38_concat_deconv')



    conv38 = convolution(conv38, 160, 3, 1, 'SAME', 'conv38_fpn')
    conv19 = convolution(conv19, 256, 3, 1, 'SAME', 'conv19_fpn')
    conv10 = convolution(conv10, 256, 3, 1, 'SAME', 'conv10_fpn')
